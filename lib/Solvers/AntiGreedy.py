...
# this class implements the anti-greedy algorithm
# always try for the worst possible option known so far
# (code is almost identical to greedy, except the name and one line in
# make_decision(): chosen_arm = np.argmin(self.Q)
    ...
    def make_decision(self):
        ...
        if self.step < self.K and self.try_each_once == True:
            ...
            if random() >= self.epsilon:
                chosen_arm = np.random.choice(np.argwhere(self.Q == np.min(self.Q)).flatten())
            else:
