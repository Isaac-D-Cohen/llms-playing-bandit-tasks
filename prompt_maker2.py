...
# (in this particular config they will never agree lol)
solver_configurations = [
    {"type": "Greedy", "name": "RealGreedy e=0", "epsilon": 0.0, "try_each_once": False, "initial_action_values": 0.0},
    {"type": "AntiGreedy", "name": "AntiGreedy e=0", "epsilon": 0.0, "try_each_once": False, "initial_action_values": 0.0},
]
...
# make a random arm on each trial that isn't any of the other arms
generate_random_alternative = False
random_arm_can_be_historical_verdict = True
...
def generate_prompts(history_file, solver_name, replicate_number, a, b, decision_history):
    ...
    for i in solver_configurations:
        ...
        elif i['type'] == "AntiGreedy":
            solvers.append(AntiGreedy(i['name'], i['epsilon'], initial_action_values=i['initial_action_values'], try_each_once=i['try_each_once']))
        elif i['type'] == 'UCB':
    ...
    for i in range(num_trials):
        ...
        for arm in arms:
            ...
            # if we don't have any endorsements yet, and the random arm can agree with the verdict
            # add the arm to the choices for random
            if len(endorsements) == 0 and random_arm_can_be_historical_verdict == True:
                unendorsed_arms.append(arm)
            ...
            elif random_arm_can_be_historical_verdict == False:
                # we only want to add it here if we didn't add it above
                unendorsed_arms.append(arm)

        if generate_random_alternative and (len(unendorsed_arms) > 0):
                ind = randint(0, len(unendorsed_arms)-1)
                arm = unendorsed_arms[ind]
                prompt[-1]['content'] = f"<Answer>{arm}</Answer>"
                filename = solver_name + ' #' + str(replicate_number) + ' step ' + str(line) + " rand_arm"
                filepath = os.path.join(output_directory, filename)
                with open(filepath, "w") as f:
                    json.dump(prompt, f)
...
if __name__ == "__main__":
    ...
    num_solvers = len(solver_configurations)+2 if generate_random_alternative else len(solver_configurations)+1
    assert (len(allow_repetition) == len(agreement) == (num_solvers))

    decision_history = [None]*(len(solver_configurations)+1)

    history_pickles = ["Random"]

    for pickle in history_pickles:
        for replicate_number in range(575):
            generate_prompts(os.path.join(input_directory, pickle + ' #' + str(replicate_number)), pickle, replicate_number, 9, 10, decision_history)
